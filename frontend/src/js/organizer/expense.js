(function () {
  ...
  function setExpenseSummary(summary) {
    ...
    if (remainingEl) {
      ...
      remainingEl.style.color = Number(summary.remaining_budget || 0) < 0 ? '#DC2626' : '#0F766E';
    }
    if (countEl) {
  ...
  function renderExpenseList(expenses, eventId) {
    ...
    if (!Array.isArray(expenses) || expenses.length === 0) {
      list.innerHTML = '<div class="empty-state"><div class="empty-state-icon">💸</div><div class="empty-state-text">No expense entries yet</div></div>';
      return;
    }
  ...
  function renderExpenseEventOptions(events) {
    ...
    select.innerHTML = events.map(event => `<option value="${event.id}">${event.title} (#${event.id})</option>`).join('');
  }
  ...
  async function loadEventExpenses(eventId) {
    ...
    try {
      ...
      if (!result.success) {
        setExpenseMessage('error', result.message || 'Unable to load expenses.');
        setExpenseSummary({ total_expenses: 0, budget: 0, remaining_budget: 0, expense_count: 0 });
  ...
  async function loadOrganizerEventsForExpenses() {
    ...
    try {
      ...
      await loadEventExpenses(Number((document.getElementById('expenseEventId') || {}).value || events[0].id));
    } catch (error) {
      console.error('Load organizer events failed:', error);
      setExpenseMessage('error', 'Failed to load your events for expense tracking.');
      setExpenseSummary({ total_expenses: 0, budget: 0, remaining_budget: 0, expense_count: 0 });
      renderExpenseList([], 0);
    }
  }
  ...
  async function handleExpenseSubmit(event) {
    ...
    const category = (document.getElementById('expenseCategory').value || 'General').trim();
    const amount = Number(document.getElementById('expenseAmount').value);
    const expenseDate = document.getElementById('expenseDate').value;
    const paymentMethod = (document.getElementById('expensePaymentMethod').value || 'Other').trim();
    const notes = (document.getElementById('expenseNotes').value || '').trim();

    if (!eventId || !title || !amount || !expenseDate) {
      setExpenseMessage('error', 'Please select event and provide title, amount and date.');
      return;
    }
    ...
    try {
      ...
      setExpenseMessage('error', 'Failed to add expense due to connection error.');
    }
  }
  ...
  async function deleteExpense(eventId, expenseId) {
    ...
    try {
      const result = await window.api.deleteEventExpense(eventId, expenseId, user.id);
      if (!result.success) {
        setExpenseMessage('error', result.message || 'Failed to delete expense.');
        return;
      }
      ...
      setExpenseMessage('error', 'Failed to delete expense due to connection error.');
    }
  }
